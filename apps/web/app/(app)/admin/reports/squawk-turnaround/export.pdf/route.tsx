...
export async function GET(req: Request) {
  ...
  const pdfRows = rows.map((r) => ({
    ...
    openedInRange: String(r.opened_in_range ?? 0),
    closedInRange: String(r.closed_in_range ?? 0),
    avgHoursToResolve: Number(r.avg_hours_to_resolve ?? 0).toFixed(1) + 'h',
  }));
  ...
  const stream = await renderToStream(
    <ReportPdfShell
      ...
      columns={[
        ...
        { key: 'avgHoursToResolve', label: 'Avg Hours to Resolve' },
      ]}
      rows={pdfRows}
    />,
  );
  ...
  return new Response(stream as unknown as ReadableStream, {
    headers: {
      ...
      'content-disposition': `attachment; filename="squawk-turnaround-${from}-${to}.pdf"`,
