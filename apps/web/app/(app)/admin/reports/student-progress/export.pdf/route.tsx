...
export async function GET(req: Request) {
  ...
  const pdfRows = rows.map((r) => {
    const abd = Number(r.ahead_behind_days ?? 0);
    return {
      name: String(r.name ?? ''),
      course: String(r.course ?? ''),
      pctComplete: Number(r.pct_complete ?? 0).toFixed(1) + '%',
      hoursFlown: Number(r.hours_flown ?? 0).toFixed(1),
      aheadBehindDays: (abd >= 0 ? '+' : '') + abd + 'd',
    };
  });
  ...
  const stream = await renderToStream(
    <ReportPdfShell
      ...
      columns={[
        ...
        { key: 'hoursFlown', label: 'Hours Flown' },
        { key: 'aheadBehindDays', label: 'Ahead/Behind' },
      ]}
      rows={pdfRows}
    />,
  );
  ...
  return new Response(stream as unknown as ReadableStream, {
    headers: {
      ...
      'content-disposition': `attachment; filename="student-progress-${from}-${to}.pdf"`,
