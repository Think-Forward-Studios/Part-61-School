...
export async function GET(req: Request) {
  ...
  const header = 'tail_number,make_model,flight_hours,scheduled_hours,utilization_pct,squawk_count';
  const lines = rows.map((r) => {
    const fh = Number(r.flight_hours ?? 0);
    const sh = Number(r.scheduled_hours ?? 0);
    const util = sh > 0 ? (fh / sh) * 100 : 0;
    return [
      escapeCsv(String(r.tail_number ?? '')),
      escapeCsv(String(r.make_model ?? '').trim()),
      fh.toFixed(1),
      sh.toFixed(1),
      util.toFixed(1),
      String(r.squawk_count ?? 0),
    ].join(',');
  });
