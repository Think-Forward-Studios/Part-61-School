...
import { aircraft, aircraftEngine, aircraftEquipment, flightLogEntry } from '@part61/db';
import {
  createAircraftInput,
  updateAircraftInput,
  aircraftIdInput,
...
export const adminAircraftRouter = router({
  ...
  create: adminProcedure.input(createAircraftInput).mutation(async ({ ctx, input }) => {
    const tx = ctx.tx as Tx;
    ...
    const rows = await tx
      ...
      .values({
        ...
        homeAirport: input.homeAirport ?? null,
      })
      .returning();
    return rows[0]!;
  }),

  update: adminProcedure.input(updateAircraftInput).mutation(async ({ ctx, input }) => {
    const tx = ctx.tx as Tx;
    ...
    if (input.equipmentNotes !== undefined) patch.equipmentNotes = input.equipmentNotes;
  ...
  softDelete: adminProcedure.input(aircraftIdInput).mutation(async ({ ctx, input }) => {
    const tx = ctx.tx as Tx;
    const rows = await tx
      .update(aircraft)
  ...
  addEngine: adminProcedure.input(addEngineInput).mutation(async ({ ctx, input }) => {
    const tx = ctx.tx as Tx;
    await loadAircraft(tx, input.aircraftId, ctx.session!.schoolId);
    const rows = await tx
  ...
  removeEngine: adminProcedure.input(removeEngineInput).mutation(async ({ ctx, input }) => {
    const tx = ctx.tx as Tx;
    const rows = await tx
  ...
  setEquipment: adminProcedure.input(setEquipmentInput).mutation(async ({ ctx, input }) => {
    const tx = ctx.tx as Tx;
    await loadAircraft(tx, input.aircraftId, ctx.session!.schoolId);
    await tx.delete(aircraftEquipment).where(eq(aircraftEquipment.aircraftId, input.aircraftId));
    if (input.tags.length > 0) {
      await tx
        .insert(aircraftEquipment)
        .values(input.tags.map((tag) => ({ aircraftId: input.aircraftId, tag })));
    }
